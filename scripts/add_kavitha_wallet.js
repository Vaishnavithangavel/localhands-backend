...
async function main() {
  try {
    ...
    const name = 'Kavitha';
    const password = 'Kavitha@123';
    const targetBalance = 500000;

    // Check if user exists
    const [users] = await pool.query(
      'SELECT u.id, u.email FROM users u JOIN user_profiles p ON u.id = p.user_id WHERE u.email = ? OR p.name LIKE ?',
      [email, `%${name}%`]
    );
    ...
    if (users.length > 0) {
      ...
      await pool.query(
        'UPDATE user_profiles SET name = ? WHERE user_id = ? AND (name IS NULL OR name = "")',
        [name, userId]
      );
    } else {
      // Create user
